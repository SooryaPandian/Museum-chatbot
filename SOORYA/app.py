...
def send_feedback(feedback: str) -> str:
    ...
    Args:
        feedback (str): The content of the feedback or report that will be sent via email.
    
    Returns:
        str: A thank you message after sending the feedback.
    """
    # Email configuration
...
model = genai.GenerativeModel(
    ...
    "If the user says any feedback or report that should be send to send_feedback function"
    "You're a chatbot that assists people in booking tickets to national museum delhi. "
    "you have to tell any questions related to museum like location,hitory, etc,..."
    ...
    "user can't say his payment is successfull.After payment ask the user for feedback"
    "As for now the museum have only 10tickets available for all days... if a person books more than that tell him ticket unavailable."
    ...
    "If the user says any feedback or report that should be send to `send_feedback` function. call that function"
    "All the messages like reports, feedbacks, or other to the museum should send to `send_feedback` function"
),
